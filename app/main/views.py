...
def measdisplay_home(tool_route):
	current_tool_routes = ["lots_obsvera401","lots_obsvera402","lots_obsverity401"]
	for i in current_tool_routes:
		if not i == "lots_obs"+tool_route:
			try:
				session.pop(i)
			except:
				_=True
	...
	if form.validate_on_submit():
		...
		lots = MeasDisplay_Obs.query.with_entities(MeasDisplay_Obs.lot, MeasDisplay_Obs.slot,MeasDisplay_Obs.recipe, MeasDisplay_Obs.port,MeasDisplay_Obs.measdate).filter(MeasDisplay_Obs.tool.like(tool_route), MeasDisplay_Obs.lot.like(lotlike), MeasDisplay_Obs.recipe.like(rcplike)).order_by(MeasDisplay_Obs.measdate.desc()).distinct().limit(100).all()
		if not len(lots) == 0:
			session['lots_obs'+tool_route] = lots
			return redirect(url_for('.measdisplay_home', tool_route=tool_route))
		flash('Query returned zero results')
...
def patternfov_home(tool_route):
	current_tool_routes = ["lots_fovvera401","lots_fovvera402","lots_fovverity401"]
	for i in current_tool_routes:
		if not i == "lots_fov"+tool_route:
			try:
				session.pop(i)
			except:
				_=True
	...
	if form.validate_on_submit():
		...
		lots = PatternFOV.query.with_entities(PatternFOV.lot, PatternFOV.slot,PatternFOV.recipe, PatternFOV.port,PatternFOV.measdate).filter(PatternFOV.tool.like(tool_route), PatternFOV.lot.like(lotlike), PatternFOV.recipe.like(rcplike)).order_by(PatternFOV.measdate.desc()).distinct().limit(100).all()
